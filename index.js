...
// Example route (e.g., in app.js or routes/index.js)
app.get("/index", (req, res) => {
  // Check if the user is logged in (req.user for Passport, req.session.username for custom sessions)
  res.sendFile(path.join(__dirname, 'index.html'));
});
...
app.get("/home", function (req, res) {
  if (req.session.loggedin) {
    console.log("Session username:", req.session.username); // Debug line
...
app.post('/register', function(req, res) {
  ...
  // Remove the nested con.connect() - use the existing connection
  var sql = "INSERT INTO register(username, email, password) VALUES (?, ?, ?)";
  ...
  con.query(sql, [username, email, password], function(error, result) {
      ...
      // Only one response - redirect to login page
  ...
  con.connect(function (error) {
    ...
    con.query(sql, [email, password], function (error, results) {
      ...
      if (results.length > 0) {
        ...
        res.redirect("/home"); // Changed from /index to /home
      } else {
